...
class MeshFactory
	...
	def build_geometry
		...
		when :sphere
			...
			segment_w = @attr[:segment_w] || 36
			segment_h = @attr[:segment_h] || 36
		...
		#　円柱ジオメトリ生成
		when :cylinder
			radius_top = @attr[:radius_top] || 1.5	
			radius_bottom = @attr[:radius_bottom] || 1.5
			height = @attr[:height] || 1
			radial_segments = @attr[:radial_segments] || 18
			height_segments = @attr[:height_segments] || 18
